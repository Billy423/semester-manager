...
**The two-tier maintenance rule, landed here and flagged:** *a target revised later than the note is evidenced staleness the agent may act on in passing, while anything else may be surfaced for confirmation and never resolved.* It is the corpus's only existing shallow/deep policy. **It was never ruled.** Its origin is marked *derived, no `[R]`*, in the one section of its source document that carries no ruling marker where every neighbouring section carries one, and adversarial review found the *act on in passing* half *"not derived from anything"* - a licence the agent granted to itself. So *leave the rule as written* is a choice to keep an unruled agent self-grant, not a choice to keep a ruling. Whether *in passing* survives *never silently* is an open decision for Billy, and its input does not exist until a kind carries a revision date (ADR `0053`).
